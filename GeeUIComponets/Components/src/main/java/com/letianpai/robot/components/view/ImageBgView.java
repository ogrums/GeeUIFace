 ...
 * @author liujunbin
 */
public class ImageBgView extends RelativeLayout {
    private Context mContext;
    private ImageView bgImage;
    private static final int UPDATE_BACKGROUND = 110;
    private UpdateViewHandler updateViewHandler;

    public ImageBgView(Context context) {
        super(context);
        init(context);
    }

    public ImageBgView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public ImageBgView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context);
    }

    public ImageBgView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
                            ...
                            try {
                                ...
                                if (info != null) {
                                    ...
                                    if (imageBgInfo != null && imageBgInfo.getData() != null && (!TextUtils.isEmpty(imageBgInfo.getData().getBg_url()))) {
                                        updateBackground(imageBgInfo.getData().getBg_url());
                                    }
