...
//        mWifiManager.removeNetwork(networkId);
    ...
    public boolean isWifiEnabled() {
        try {
            ...
            Log.e(TAG, "isWifiEnabled: "+e.getMessage());
            return false;
        }
    }
    ...
    public boolean isConnected(String ssid) {
        ...
        switch (wifiInfo.getSupplicantState()) {
            ...
            case COMPLETED:
                Log.e("auto_connect"," wifiInfo.getSSID(): "+ wifiInfo.getSSID().replace("\"", "").toString());
...
//        }
    ...
    public int getConnectState(Context context, String SSID) {
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            ...
            if (wifiInfo != null) {
                Log.e("","getConnectState()..connectState: " + connectState + "；ossid: " + SSID + ":wifiinfo ssid: " + wifiInfo.getSSID());
                if (wifiInfo.getSSID().equals("\"" + SSID + "\"") || wifiInfo.getSSID().equals(SSID)) {

                    if (networkInfo != null && networkInfo.getType() == ConnectivityManager.TYPE_WIFI) {
                        Log.e("","getConnectState().. network type: " + networkInfo.getType());
    ...
    public static boolean isWifiConnected() {
        if (sInstance.mContext != null) {
            ConnectivityManager mConnectivityManager = (ConnectivityManager) sInstance.mContext
                    .getSystemService(Context.CONNECTIVITY_SERVICE);
