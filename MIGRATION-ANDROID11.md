...
## Sous-modules GeeUIBase / GeeUIComponets

Le `settings.gradle` d'origine pointait vers `GeeUIComponents/` (mauvais nom).
Corrigé vers `GeeUIComponets/`.

Les deux sous-modules sont **vendored** dans ce fork (plus de `git submodule update`).
Leurs `build.gradle` sont au même niveau AGP 8.13 / minSdk 30.
